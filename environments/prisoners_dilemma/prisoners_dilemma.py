...
# TODO: Add final round warning in build round context / within is_final_turn
...
class PrisonersDilemmaEnvironment(MultiAgentEnvironment):
	def __init__(self, config: EnvironmentConfig):
		...
		self.round_number = 0
		self.agent_points = {
			name: self.starting_points for name in config.agent_name if name not in self.wardens
		}

		self.messages = {
			'private': {
				(sender, receiver): []
				for sender in config.agent_name
				for receiver in config.agent_name
				if sender != receiver
			},
			'public': [],
		}
	...
	def _build_round_payoff_breakdown_history(self) -> List[Dict[str, Any]]:
		"""Build completed-round payoff breakdown history in round order."""
		history = []
		for round_num in sorted(self.past_round_payoff_breakdowns.keys()):
			history.append(
				{
					'round': round_num,
					'payoff_breakdown': deepcopy(self.past_round_payoff_breakdowns[round_num]),
				}
			)
		return history

	def _build_round_recruitment_results_history(self) -> List[Dict[str, Any]]:
		"""Build completed-round recruitment results history in round order."""
		history = []
		for round_num in sorted(self.past_round_recruitment_results.keys()):
			history.append(
				{
					'round': round_num,
					'recruitment_results': deepcopy(self.past_round_recruitment_results[round_num]),
				}
			)
		return history
	...
	def _build_all_prisoners_decision_history(self) -> Dict[str, List[Dict[str, Any]]]:
		...
		return {
			prisoner_id: self._build_prisoner_decision_history(prisoner_id)
			for prisoner_id in self.prisoners
		}
	...
	def _build_prisoner_decision_history(self, prisoner_id: str) -> List[Dict[str, Any]]:
		...
		for round_num in sorted(self.past_round_deliberation_logs.keys()):
			for entry in self.past_round_deliberation_logs[round_num]:
				turn_entry = {
					'round': round_num,
					'turn': entry['turn'],
		...
		for entry in self.deliberation_log:
			...
			# extract advice given to this prisoner from each warden this turn
			for warden_id, advice_dict in entry.get('warden_advice', {}).items():
				if isinstance(advice_dict, dict):
					turn_entry['warden_advice'][warden_id] = advice_dict.get(prisoner_id)
				else:
	...
	def step(
		...
		agent_cot = {
			agent_id: self._extract_agent_cot(agent_id) for agent_id in self.config.agent_name
		}
		...
		self.deliberation_log.append(
			{
				...
				'agent_private_messages_to_wardens': {
					p: {
						w: self._extract_private_message(
				...
				'warden_private_messages_to_agents': {
					w: {
						p: self._extract_private_message(
		...
		if not is_final_turn:
			...
			next_observations = {
				...
				for aid in self.config.agent_name
			}
			rewards = {p: 0.0 for p in self.config.agent_name}
	...
	def _parse_all_actions(self, agent_actions: Mapping[str, Any]) -> Dict[str, Dict[str, Any]]:
		parsed = {}
		for name in self.config.agent_name:
			if name in self.wardens:
				parsed[name] = self._parse_warden_action(agent_actions, name)
			else:
				parsed[name] = self._parse_prisoner_action(agent_actions, name)
		return parsed

	def _parse_prisoner_action(
		self, agent_actions: Mapping[str, Any], agent_id: str
	) -> Dict[str, Any]:
		...
		if isinstance(action, str):
			...
			try:
				...
				logger.error(
					f'[{agent_id}] Final JSON parse failed: {e}, action={str(action)[:300]}'
				)
	...
	def _build_step_info(
		...
		return {
			...
			'payoffs': {
				**{p: payoffs[p] for p in self.prisoners},
				**{w: 0.0 for w in self.wardens},
			},
	...
	def reset(self) -> Mapping[str, Dict[str, Any]]:
		...
		self.messages = {
			'private': {
				(sender, receiver): []
				for sender in self.config.agent_name
				for receiver in self.config.agent_name
				if sender != receiver
			},
			'public': [],
		}
		...
		return {
			...
			for agent_id in self.config.agent_name
		}

	def _extract_private_message(
		self, private_messages_dict: Dict[str, Any], recipient: str
	) -> Optional[str]:
