		...
		for attempt in range(1, self.max_retries + 1):
			try:
				...
				if attempt < self.max_retries:
					...
					continue
				logger.error(
					f'[OpenRouterClient] Network error after {self.max_retries} attempts: {exc}'
				)
				return empty_result
			...
			if gen_result.content:
				logger.debug(
					f'[OpenRouterClient] Response text: {gen_result.content[:500]}, Response type: {type(gen_result.content)}'
				)
			else:
				...
				if attempt < self.max_retries:
					...
					continue
				logger.warning(
					f'[OpenRouterClient] Empty response after {self.max_retries} attempts: {result}'
				)
			return gen_result

		logger.error(
			f'[OpenRouterClient] All {self.max_retries} retries exhausted. Last error: {last_error}'
		)
		return empty_result
